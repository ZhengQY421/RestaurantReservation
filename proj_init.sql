...
CREATE OR REPLACE FUNCTION delete_reservation_update()
RETURNS TRIGGER AS
$$
BEGIN
    UPDATE Tables
    SET reserveId = NULL, vacant = true
    WHERE reserveid = OLD.reserveid;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER delete_reservation_update
BEFORE DELETE OR UPDATE ON Reserves
FOR EACH ROW
EXECUTE PROCEDURE delete_reservation_update();
